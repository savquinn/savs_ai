...
def explore_chat_db(db_path):
    ...
    try:
        ...
        print("\n=== Message Types ===")
        try:
            cursor.execute("SELECT service, COUNT(*) as count FROM message GROUP BY service")
        ...
        try:
            ...
            if not top_chats.empty:
                for _, row in top_chats.iterrows():
                    print(f"{row['display_name'] or 'Unnamed Chat'}: {row['message_count']:,} messages")
        ...
        try:
            ...
            if not sample_messages.empty:
                for _, row in sample_messages.iterrows():
                    print(f"[{row['date']}] {row['sender']}: {row['text_sample']}{'...' if len(row['text_sample']) == 50 else ''}")
        except Exception as e:
        ...
        try:
            ...
            if not message_lengths.empty:
                ...
                print("Created visualization: chat_db_analysis/message_length_distribution.png")
        except Exception as e:
